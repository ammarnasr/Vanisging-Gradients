...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        for i in named_parameters:
            layer_name = i[0].replace("layer_dict.", "")
            grad_tensor = i[1]._grad
            abs_mean_grad = torch.mean(grad_tensor).abs().item()
            if layer_name[-4:] == 'bias':
                continue
                abs_mean_grad = torch.mean(grad_tensor).abs().item()
                all_grads[-1]= np.mean(all_grads[-1]+abs_mean_grad)
                continue
            layer_name = i[0].replace(".weight", "")
            layer_name = i[0].replace(".", "_")
